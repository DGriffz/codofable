...
4. **This repo's history is two commits.** [repo] Do not cite project history as evidence for anything in this file; there is none. The evidence base is external literature ([doc], cited) and first-principles reasoning ([craft], labeled).
...
- **Generic coding benchmarks are contaminated.** SWE-bench Verified and its source repositories are public and heavily discussed; OpenAI announced it stopped reporting the benchmark, citing contamination difficulty and a manual audit in which the majority of examined model failures traced to test flaws rather than model limitations ([doc] https://openai.com/index/why-we-no-longer-evaluate-swe-bench-verified/). Independent analysis (SWE-Bench+) reports widespread solution leakage — the fix visible or hinted in the issue text — and weak test suites that let wrong patches count as resolved ([doc] https://arxiv.org/abs/2410.06992). Further work found models reproducing benchmark-specific details from memory rather than reasoning ([doc] https://arxiv.org/html/2506.12286v4). A contaminated benchmark cannot isolate the effect of a skill library: memorized solutions swamp the treatment.
...
**Asset.** Sixteen skills with deliberately trigger-rich descriptions, a fixed inventory (Section 4 names are final), and an explicit "When to use / When NOT to use" contract in every skill — i.e., ground-truth routing labels already exist for every skill; they just have not been tested against behavior. [craft]
...
- [repo] Repository state (two commits, 16-skill inventory, no experiment artifacts exist yet). Re-verify: `git -C . log --oneline` and `ls .claude/skills/` from the repo root.
- [doc] SWE-bench contamination/test-flaw claims: https://openai.com/index/why-we-no-longer-evaluate-swe-bench-verified/ ; solution leakage and weak tests: https://arxiv.org/abs/2410.06992 (SWE-Bench+) ; memorization evidence: https://arxiv.org/html/2506.12286v4 . Accessed via web search 2026-07-05; specific percentages deliberately omitted — re-read the sources before quoting numbers publicly.
...
Drift triggers for this file: a completed run of any P1–P5 experiment; a change to the skill inventory (P5 subsets reference Section-4 names); a Claude Code release changing skill loading or hooks (invalidates P2's measurement plan); public retirement or replacement of the cited benchmarks (weakens but does not invalidate P1's "why SOTA fails" argument).
