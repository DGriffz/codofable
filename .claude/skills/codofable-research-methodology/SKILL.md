...
| **RETIRED** (terminal) | Refuted by decision rule, broken by refuter, or overtaken — at any state past hunch | — | An entry in `codofable-failure-archaeology`: symptom/idea → why retired → **the evidence** → status. Flag removed or marked dead per `codofable-config-mapping` |
